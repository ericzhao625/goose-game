/*
START OF gooseEscapeActors.cpp
*/
#include <iostream>
#include <cmath>
...
Protagonist::Protagonist(int x0, int y0)
{
	actorChar = PLAYER_CHAR;
	update_location(x0,y0);
}
...
Flier::Flier(int x0, int y0)
{
    actorChar = GOOSE_CHAR;
    update_location(x0,y0);
}

void Flier::chase(Actor & player, int game_board[NUM_BOARD_Y][NUM_BOARD_X], int turn)
{
...
void Bouncer::bounce(int game_board[NUM_BOARD_Y][NUM_BOARD_X])
{
	int xMove = 0, yMove = 0;
